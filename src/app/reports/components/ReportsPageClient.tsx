...
function ReportPreviewPanel({ report, onClose }: { report: Report; onClose: () => void }) {
  ...
  const { activeClientId, activeClientName, clients } = useActiveClient();
  const activeClientNpwp = clients.find(c => c.id === activeClientId)?.npwp;
            ...
            <div className="max-w-2xl mx-auto bg-card rounded-xl shadow-sm border border-border overflow-hidden">
              ...
              <div className="bg-primary px-8 py-6 text-white">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-xs font-medium opacity-70 uppercase tracking-wider mb-1">{activeClientName ?? 'No client selected'}</p>
              ...
              <div className="px-8 py-4 bg-muted/20 border-t border-border flex items-center justify-between">
                <p className="text-[10px] text-muted-foreground">{activeClientName ?? 'No client selected'}{activeClientNpwp && activeClientNpwp !== '-' ? ` · NPWP ${activeClientNpwp}` : ''}</p>
