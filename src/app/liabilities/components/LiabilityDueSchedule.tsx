...
// Backend integration point: replace with API call to /api/liabilities/due-schedule?period=...
const obligations = [
  { id: 'OBL-2026-001', liability: 'PPh 21 Agustus 2026', type: 'Tax Payable', creditor: 'Direktorat Jenderal Pajak', dueDate: '8 Sep 2026', amount: 'Rp 182M', daysRemaining: 13, status: 'due-soon' as const },
  { id: 'OBL-2026-002', liability: 'Hutang Dagang — PT Sinar Abadi', type: 'Accounts Payable', creditor: 'PT Sinar Abadi Makmur', dueDate: '15 Sep 2026', amount: 'Rp 142M', daysRemaining: 20, status: 'due-soon' as const },
  { id: 'OBL-2026-003', liability: 'Cicilan KPR Gedung Surabaya', type: 'Long-Term Debt', creditor: 'Bank Mandiri', dueDate: '1 Oct 2026', amount: 'Rp 48M', daysRemaining: 36, status: 'scheduled' as const },
  { id: 'OBL-2026-004', liability: 'Hutang Dagang — CV Maju Jaya', type: 'Accounts Payable', creditor: 'CV Maju Jaya Teknik', dueDate: '10 Oct 2026', amount: 'Rp 88M', daysRemaining: 45, status: 'scheduled' as const },
  { id: 'OBL-2026-005', liability: 'PPN Masa Agustus 2026', type: 'Tax Payable', creditor: 'Direktorat Jenderal Pajak', dueDate: '25 Sep 2026', amount: 'Rp 62M', daysRemaining: 30, status: 'scheduled' as const },
  { id: 'OBL-2026-006', liability: 'Hutang Bank BRI — Kredit Modal Kerja', type: 'Short-Term Debt', creditor: 'Bank Rakyat Indonesia', dueDate: '20 Nov 2026', amount: 'Rp 120M', daysRemaining: 86, status: 'scheduled' as const },
  { id: 'OBL-2026-007', liability: 'Sewa Kantor Bandung — Q4 2026', type: 'Lease Liability', creditor: 'PT Graha Properti Nusantara', dueDate: '1 Oct 2026', amount: 'Rp 24M', daysRemaining: 36, status: 'scheduled' as const },
  { id: 'OBL-2026-008', liability: 'Hutang Dagang — PT Bintang Mas', type: 'Accounts Payable', creditor: 'PT Bintang Mas Sejahtera', dueDate: '28 Aug 2026', amount: 'Rp 58M', daysRemaining: 2, status: 'overdue' as const },
  { id: 'OBL-2026-009', liability: 'Cicilan Leasing Forklift', type: 'Lease Liability', creditor: 'BFI Finance Indonesia', dueDate: '5 Sep 2026', amount: 'Rp 8.4M', daysRemaining: 10, status: 'due-soon' as const },
  { id: 'OBL-2026-010', liability: 'Hutang Obligasi — BNI', type: 'Long-Term Debt', creditor: 'Bank Negara Indonesia', dueDate: '15 Mar 2028', amount: 'Rp 500M', daysRemaining: 566, status: 'scheduled' as const },
];

export default function LiabilityDueSchedule() {
    ...
    <div className="fin-card mb-6">
          ...
          <tbody>
            {paginated.map(o => (
              <tr key={`due-row-${o.id}`} className="border-b border-border hover:bg-muted/30 transition-colors group">
                ...
                <td className="px-4 py-3 font-600 text-foreground financial-value whitespace-nowrap">{fx(o.amount)}</td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`text-[12px] font-600 financial-value ${o.daysRemaining <= 0 ? 'text-negative' : o.daysRemaining <= 14 ? 'text-negative' : o.daysRemaining <= 30 ? 'text-warning' : 'text-muted-foreground'}`}>
                    {o.daysRemaining <= 0 ? 'Overdue' : `${o.daysRemaining} days`}
                  </span>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
      ...
      <div className="px-5 py-3 border-t border-border flex items-center justify-between">
        <div className="text-[11px] text-muted-foreground">
          Showing {(page - 1) * perPage + 1}–{Math.min(page * perPage, filtered.length)} of {filtered.length} obligations
        </div>
