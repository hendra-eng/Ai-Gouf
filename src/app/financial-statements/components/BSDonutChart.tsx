...
function CustomTooltip({ active, payload, currency }: { active?: boolean; payload?: { name: string; value: number; payload: { pct: number } }[]; currency: 'IDR' | 'USD' | 'SGD' }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-card-lg p-3">
      ...
      <p className="text-xs text-muted-foreground">{d.payload.pct.toFixed(1)}% of total</p>
    </div>
  );
}
...
export default function BSDonutChart({ totalAssets, currentAssets, nonCurrentAssets, totalLiabilities, totalEquity }: BSDonutChartProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <p className="text-xs font-semibold text-center text-muted-foreground mb-2">Asset Mix</p>
        <ResponsiveContainer width="100%" height={200}>
          <PieChart>
            ...
            <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, fontFamily: 'var(--font-plus-jakarta-sans)' }} />
          </PieChart>
        </ResponsiveContainer>
      </div>
      <div>
        <p className="text-xs font-semibold text-center text-muted-foreground mb-2">Funding Structure</p>
        <ResponsiveContainer width="100%" height={200}>
          <PieChart>
