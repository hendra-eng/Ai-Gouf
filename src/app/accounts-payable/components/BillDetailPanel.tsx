...
interface Props {
  bill: Bill;
  onClose: () => void;
  // [BARU] Menulis balik ke transaksi Expense sumber Bill ini lewat
  // saveEdit() di TransactionsContext — sebelumnya kedua tombol ini cuma
  // menampilkan toast tanpa mengubah data apapun.
  onMarkPaid: () => void;
  onSchedule: (newDueDateISO: string) => void;
}

export default function BillDetailPanel({ bill, onClose, onMarkPaid, onSchedule }: Props) {
  const { fx } = useCurrency();
  const router = useRouter();
  const [note, setNote] = useState('');
  const [showScheduleForm, setShowScheduleForm] = useState(false);
  const [scheduleDate, setScheduleDate] = useState(bill.dueDate);
      ...
      >
        <div className="sticky top-0 bg-card border-b border-border px-5 py-4 z-10">
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <button onClick={() => setShowScheduleForm(!showScheduleForm)} className="flex items-center gap-1.5 text-xs font-medium text-primary bg-primary/10 hover:bg-primary/20 rounded-md px-2.5 py-1.5 transition-colors">
              <Icon name="CalendarIcon" size={12} />
              Schedule Payment
            </button>
            <button onClick={onMarkPaid} className="flex items-center gap-1.5 text-xs font-medium text-success bg-success-bg hover:bg-green-100 rounded-md px-2.5 py-1.5 transition-colors">
              <Icon name="CheckCircleIcon" size={12} />
              Mark Paid
            </button>
            <button onClick={() => router.push('/ai-financial-analyst?analysis=ap-risk')} className="flex items-center gap-1.5 text-xs font-medium text-ai-purple bg-ai-purple-bg hover:bg-purple-100 rounded-md px-2.5 py-1.5 transition-colors">
              <Icon name="SparklesIcon" size={12} />
              AI Risk
            </button>
            <button onClick={() => toast.success('Bill exported as PDF')} className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground border border-border rounded-md px-2.5 py-1.5 hover:bg-secondary transition-colors">
        ...
        <div className="p-5 space-y-4">
          {showScheduleForm && (
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-4 slide-up">
              <h4 className="text-sm font-semibold text-foreground mb-3">Schedule Payment</h4>
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-muted-foreground block mb-1">Amount (IDR)</label>
                  <input
                    type="number"
                ...
                <div className="flex gap-2">
                  <button onClick={handleSchedule} className="flex-1 bg-primary text-white text-sm font-medium rounded-md py-2 hover:bg-primary/90 transition-colors">Save Schedule</button>
                  <button onClick={() => setShowScheduleForm(false)} className="px-4 text-sm font-medium text-muted-foreground border border-border rounded-md hover:bg-secondary transition-colors">Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            <h4 className="text-sm font-semibold text-foreground mb-3">Bill Summary</h4>
            <div className="space-y-2 text-sm">
              {[
                ...
                { label: 'Outstanding', value: fx(formatRupiah(bill.outstanding, true)), color: bill.outstanding > 0 ? 'text-danger font-bold' : 'text-success' },
              ].map((row) => (
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            <h4 className="text-sm font-semibold text-foreground mb-3">Bill Details</h4>
            <div className="space-y-2 text-sm">
              {[
                { label: 'Bill Date', value: bill.billDate },
                { label: 'Due Date', value: bill.dueDate },
                { label: 'Days Overdue', value: bill.daysOverdue > 0 ? `${bill.daysOverdue} days` : 'Not overdue', color: bill.daysOverdue > 0 ? 'text-danger font-semibold' : 'text-success' },
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => toast.info('Note saved')} className="text-xs text-primary hover:underline font-medium">Save</button>
            </div>
            <textarea
              value={note}
