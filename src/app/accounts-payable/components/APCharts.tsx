...
import { apAgingData, apTrendData, paymentForecastData, vendors, formatRupiah } from '@/lib/mockData';
...
export default function APCharts() {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 2xl:grid-cols-3 gap-4">
      ...
      <div className="xl:col-span-2 bg-card border border-border rounded-lg p-5 shadow-card">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-md font-600 text-foreground">AP Aging Analysis</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Total AP: {fx('Rp 860M')} outstanding</p>
          </div>
          <span className="text-xs text-danger font-500 bg-danger-bg px-2 py-1 rounded-full">{fx('Rp 96M')} overdue</span>
        </div>
      ...
      <div className="bg-card border border-border rounded-lg p-5 shadow-card">
        <h3 className="text-md font-600 text-foreground mb-1">Upcoming Payment Obligations</h3>
          ...
          {paymentForecastData.map((pf) => (
            <div key={`apf-${pf.period}`} className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="font-500 text-foreground">{pf.period}</span>
                <span className="tabular-nums font-600">{fx(formatRupiah(pf.amount, true))}</span>
              </div>
              <div className="h-2 bg-secondary rounded-full overflow-hidden">
                <div
                  className="h-full bg-primary rounded-full"
                  style={{ width: `${(pf.amount / 320000000) * 100}%` }}
                />
              </div>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground font-500">Total 30-day obligation</span>
            <span className="font-700 text-foreground tabular-nums">{fx('Rp 320M')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="xl:col-span-2 bg-card border border-border rounded-lg p-5 shadow-card">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-md font-600 text-foreground">AP Trend</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Monthly AP movement Jan–Aug 2026</p>
          </div>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={apTrendData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
            <defs>
      ...
      {/* Vendor Exposure */}
      <div className="bg-card border border-border rounded-lg p-5 shadow-card">
        <h3 className="text-md font-600 text-foreground mb-1">Top Vendor Exposure</h3>
